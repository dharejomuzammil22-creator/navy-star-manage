...
import { Filter, Search, Download, Pencil, Printer } from "lucide-react";

export const Route = createFileRoute("/admin/trainer-attendance/view")({
...
function ViewTrainerAttendance() {
  ...
  return (
    <>
      <PageHeader title="View Trainer Attendance" subtitle="Daily check-in / check-out logs for all trainers." />
      <SectionCard title="Attendance Log">
        <div className="flex flex-wrap items-center justify-end gap-2 mb-4">
          <button onClick={() => window.print()} className="h-9 w-9 grid place-items-center rounded-full border border-border hover:bg-accent" aria-label="Print" title="Print">
            <Printer className="h-4 w-4" />
          </button>
          <button onClick={() => setShowFilters((v) => !v)} className="h-9 inline-flex items-center gap-1.5 px-3 rounded-md border border-border text-sm hover:bg-accent">
          ...
          <button onClick={exportCsv} className="h-9 inline-flex items-center gap-1.5 px-3 rounded-md bg-primary text-primary-foreground text-sm hover:opacity-90">
        ...
        {showFilters && (
          <div className="mb-4 rounded-md border border-border bg-muted/30 p-3 flex flex-wrap items-center gap-3">
            ...
            <button onClick={() => { setCampusFilter(""); setQ(""); }} className="ml-auto text-xs text-primary hover:underline">Clear</button>
          </div>
        )}
            ...
            <tbody>
              {filtered.map((r) => (
                <tr key={r.id} className="border-t border-border hover:bg-muted/40 align-top">
                  ...
                  <td className="px-4 py-4">
                    {r.checkIn ? (
                      <div className="space-y-1">
                        ...
                        {r.checkIn.late && (
                          <span className="inline-block rounded bg-destructive/15 text-destructive text-[11px] px-1.5 py-0.5">Late: {r.checkIn.late}</span>
                        )}
                      </div>
                  ...
                  <td className="px-4 py-4">
                    {r.checkOut ? (
                      <div className="space-y-1"><div>{r.checkOut.date},</div><div>{r.checkOut.time}</div></div>
                    ) : (<span className="text-muted-foreground">—</span>)}
                  </td>
                  <td className="px-4 py-4 tabular-nums">{r.duration}</td>
                  <td className="px-4 py-4">
                    <span className="inline-flex items-center rounded border border-primary/40 text-primary bg-primary/5 text-xs px-2 py-0.5 capitalize">{r.status}</span>
                  </td>
                  <td className="px-4 py-4">
                    <button onClick={() => editRow(r.id)} className="h-8 w-8 grid place-items-center rounded-md hover:bg-accent text-muted-foreground hover:text-foreground" aria-label="Edit" title="Edit status">
